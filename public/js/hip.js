...
jQuery.fn.hip = function(param) {
    ...
    if(objects.length>0){
        ...
        jQuery.each( objects, function( key,curObj ) {
            ...
            //oculto la paginacion si hay menos objetos que objetos mostrados por pagina

            if(filter){

                /*jQuery(".hip-buscador").append('<div class="hip-filter  mt-3">\n' +
            ...
            //Filters
            //oculto la paginacion si solo hay una página
            if(page_length <= 1){
                $(".hip-pagination").addClass('d-none');
            }
        });
...
function showPage(elem) {
    ...
    //var hip_items = jQuery(elem).find(".hip-item").not(".d-none");
...
jQuery(document).on('keyup','.hip-search-input', function () {
    actualizarPaginacion($(this).val());
});

function actualizarPaginacion(elem, tipo = null){
    var obj = jQuery("#grid-elements");
    var objPagination = jQuery(".hip-pagination");
    var itemsPerPage = obj.attr('data-item-per-page');
    var items = obj.find(".hip-item").not('.d-none');
    var filteredLen = Math.ceil(getFilteredCount(items,elem)/itemsPerPage);
    if(elem === "" && tipo === null){
    ...
    $(".hip-pagination").find("a[href='#1']").click();

    $(".hip-pagination").addClass('hip-item-hidden');
    if(filteredLen > 1){
        $(".hip-pagination").removeClass('hip-item-hidden');
    }
}
...
function getFilteredCount(items,query) {
    ...
    items.each(function() {
        var contador_coincidencias = 0;
        var card = jQuery(this);
        var tags = jQuery(this).attr('data-tags').trim();
        var querySplited = value.trim().split(" ");
        if(typeof tags !== "undefined")
        {
            var coincidencias = [];
            tags = tags.toLowerCase().split(" ");
            jQuery.each(tags,function (index,item) {
                jQuery.each(querySplited,function (index1,item1) {
                    if (item1 !== " ") {
                        if (item1 == item) {
                            target = true;
                            coincidencias.push(true);
                        } else if (item.indexOf(item1) > -1) {
                            target = true;
                            coincidencias.push(true);
                        } else {
                            target = false;
                            coincidencias.push(false);
                        }
                    }
            ...
            //var contador_coincidencias = 0;
            $.each(coincidencias, function(index, item){
                if(item){
                    contador_coincidencias ++;
                }
            });
            if(contador_coincidencias >= querySplited.length){
                init_filter++;
